...
- `recommend_storefront` changes no result data, but it immediately applies the browser-authored `workingHeadline` and starts a paint-guaranteed progress overlay while retrieval and curation run. It should normally be followed by `curate_storefront_results`; stop at a plain search list only when the user explicitly asks for raw or conventional results. For an eligible self-directed request it offers available library personalization once; declined, immediate-answer, unavailable, gift, and group flows use public data with no taste personalization. It supports reference/include/preferred/excluded-tag intent scoring, filters owned games inside the page, and returns only public game records plus an exclusion count. Clear restores the default headline.
- `curate_storefront_results` is the preferred presentation step after retrieval. It stages a headline, summary, featured badges, per-game reasons, and ordering after validating every app ID against the originating recommendation.
