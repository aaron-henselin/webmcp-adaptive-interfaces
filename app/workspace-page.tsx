...
function HtmlWidget({ block, recordCount }: { block: HtmlBlock; recordCount: number }) {
  const safeMarkup = useMemo(() => renderHtmlWidget(block.markup, { pageTitle: PAGE_TITLE, recordCount, userFirstName: USER_FIRST_NAME }), [block.markup, recordCount]);
...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    ...
    const tools = [
      { name: "describe_steam_catalog", description: "Describe the Steam catalog, current page, and the built-in composition guide. Read pageCompositionGuide before building a page so widths, hierarchy, personalization, tabs, and calls to action are chosen from the user's goal.", inputSchema: { type: "object", additionalProperties: false, properties: {} }, annotations: { readOnlyHint: true, untrustedContentHint: false }, execute: () => { const current = workspaceRef.current!; return { content: [{ type: "text", text: `Described ${CATALOG_FIELD_CATALOG.length} reportable fields and ${workspaceOutline(current).length} page blocks.` }], structuredContent: { schemaVersion: "steam-desk.workspace/v1", source: { name: "steam_catalog", recordCount }, fields: CATALOG_FIELD_CATALOG, workspace: { storage: "local", selectedBlockId: current.selectedBlockId, blocks: workspaceOutline(current) }, htmlBindings: HTML_BINDINGS, compositionGuide: PAGE_COMPOSITION_GUIDE, spans: SPANS, composeOperations: ["inspect", "select", "addHtml", "addTabs", "move", "setSpan", "configure", "remove", "undo", "reset"], reportDefinition: { data: REPORT_DATA_SCHEMA, presentation: REPORT_PRESENTATION_SCHEMA } } }; } },
      { name: "create_report", description: "Create a database-backed report and place it inline. Infer its span from the composition guide unless the user explicitly specifies layout: full for primary or dense content, half for paired peers, third for compact KPIs.", inputSchema: { type: "object", additionalProperties: false, properties: { title: { type: "string", maxLength: 100 }, description: { type: "string", maxLength: 220 }, span: { type: "string", enum: SPANS, description: "Choose from the composition guide: full for primary/dense content, half for paired peers, third for compact KPIs." }, data: REPORT_DATA_SCHEMA, presentation: REPORT_PRESENTATION_SCHEMA, openInBrowser: { type: "boolean", default: true } }, required: ["title", "data", "presentation"] }, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: async (input: Record<string, unknown>) => { try { const created = await createReport(input); return { content: [{ type: "text", text: `Created “${created.report.title}” and placed it on the page.` }], structuredContent: { schemaVersion: "steam-desk.report-receipt/v4", ok: true, report: { id: created.report.id, title: created.report.title, mode: created.report.presentation.mode, span: created.report.span, rowCount: created.result.rows.length }, workspace: { storage: "local", selectedBlockId: created.report.id } } }; } catch (error) { return { isError: true, content: [{ type: "text", text: error instanceof Error ? error.message : "Report creation failed." }] }; } } },
      { name: "compose_page", description: "Inspect or immediately compose the local page. Apply pageCompositionGuide proactively: infer widths and hierarchy from the user's goal, use restrained personalization for briefings or home pages, and include one useful CTA when appropriate. The user should not need to specify layout. Use target 'selected' for the browser-selected block.", inputSchema: COMPOSE_PAGE_SCHEMA, annotations: { readOnlyHint: false, untrustedContentHint: false }, execute: async (input: Record<string, unknown>) => { try { const operations = normalizeOperations(input.operations); const current = workspaceRef.current; if (!current) throw new Error("The page workspace is unavailable."); if (operations.some((operation) => operation.op === "undo")) { if (operations.length !== 1) throw new Error("undo must be the only page operation."); const changed = undoWorkspace(); const restored = workspaceRef.current!; return { content: [{ type: "text", text: changed ? "Undid the last page change." : "There is no page change to undo." }], structuredContent: { ok: true, changed, compositionGuide: PAGE_COMPOSITION_GUIDE, workspace: { storage: "local", selectedBlockId: restored.selectedBlockId, blocks: workspaceOutline(restored) } } }; } const applied = applyOperations(current, operations); if (applied.changes.length) commitWorkspace(applied.workspace, !operations.every((operation) => operation.op === "select")); if (input.openInBrowser !== false && applied.changes.length) window.setTimeout(() => workspaceSectionRef.current?.scrollIntoView({ behavior: "smooth", block: "start" }), 80); return { content: [{ type: "text", text: applied.changes.length ? applied.changes.join(" ") : `The page contains ${workspaceOutline(applied.workspace).length} top-level blocks.` }], structuredContent: { schemaVersion: "steam-desk.compose-receipt/v1", ok: true, changed: Boolean(applied.changes.length), changes: applied.changes, compositionGuide: PAGE_COMPOSITION_GUIDE, workspace: { storage: "local", selectedBlockId: applied.workspace.selectedBlockId, blocks: workspaceOutline(applied.workspace) } } }; } catch (error) { return { isError: true, content: [{ type: "text", text: error instanceof Error ? error.message : "Page composition failed." }], structuredContent: { ok: false } }; } } },
