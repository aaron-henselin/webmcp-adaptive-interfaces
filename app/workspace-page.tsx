  ...
  properties: {
    operations: {
      type: "array", maxItems: 16, items: {
        ...
        properties: {
          op: { type: "string", enum: ["inspect", "setPageTitle", "select", "addHtml", "addTabs", "move", "setSpan", "configure", "remove", "undo", "reset"] },
          target: { type: "string", maxLength: 128 }, title: { type: "string", maxLength: 100, description: "The page title for setPageTitle, or the block title for add and configure operations." }, markup: { type: "string", maxLength: MAX_HTML_LENGTH, description: "Presentation-only HTML for a widget. Do not include Markdown links, anchor elements, href attributes, linked URLs, or button-like links. Express any next step as plain text." },
...
function markdownReport(report: ReportBlock, result: ReportResult) {
  ...
  if (presentation.mode === "metric" || presentation.mode === "mixed") lines.push(`**${presentation.metric.label}:** ${formatValue(result.rows[0]?.[presentation.metric.valueField], presentation.metric.format)}`);
    ...
    const columns = presentation.mode === "table" ? presentation.table.columns : Object.keys(result.rows[0] ?? {}).slice(0, 8).map((field) => ({ field, label: field, format: field === "releaseYear" ? "year" as ValueFormat : "number" as ValueFormat }));
...
function HtmlWidget({ block, pageTitle, recordCount, firstName, jobRole, company }: { block: HtmlBlock; pageTitle: string; recordCount: number; firstName: string; jobRole: string; company: string }) {
  const safeMarkup = useMemo(() => renderHtmlWidget(block.markup, { pageTitle, recordCount, userFirstName: firstName, userJobRole: jobRole, userCompany: company }), [block.markup, company, firstName, jobRole, pageTitle, recordCount]);
...
function normalizeOperations(value: unknown): WorkspaceOperation[] {
  ...
  return value.slice(0, 16).map((item): WorkspaceOperation => {
    ...
    if (item.op === "setPageTitle") { const title = text(item.title, "", 100); if (!title) throw new Error("setPageTitle requires a title."); return { op: "setPageTitle", title }; }
...
export default function WorkspacePage({ webMcpStatus, onWebMcpStatusChange }: { webMcpStatus: WebMcpStatus; onWebMcpStatusChange: (status: WebMcpStatus) => void }) {
  ...
  const pageTitle = workspace?.pageTitle ?? "Untitled page";
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (input.operation === "start") {
          return {
            ...
            structuredContent: {
              schemaVersion: "adaptive-interfaces.audience-onboarding/v1",
        ...
        if (!company && result.resolution.status === "ambiguous") {
          return {
            ...
            structuredContent: { schemaVersion: "adaptive-interfaces.audience-onboarding/v1", stage: "company_clarification", submitted: { firstName, company: companyQuery, jobRole }, candidates: result.resolution.alternatives, instruction: "Ask only which listed company they mean. Do not make an ambiguous employer choice." },
          };
        }
        if (!company) {
          return {
            ...
            structuredContent: { schemaVersion: "adaptive-interfaces.audience-onboarding/v1", stage: "company_not_found", submitted: { firstName, company: companyQuery, jobRole }, candidates: result.candidates, instruction: "Do not invent a company or silently substitute a weak match." },
          };
        }
    ...
    const tools = [
      { name: "describe_page_data", description: "Describe the builder's reportable data, page title, page outline, audience, onboarding stage, approved proposal, composition guide, and exact presentation modes. If onboarding is incomplete, use onboard_audience instead of collecting or saving identity through compose_page.", inputSchema: { type: "object", additionalProperties: false, properties: {} }, annotations: { readOnlyHint: true, untrustedContentHint: false }, execute: () => { const current = workspaceRef.current!; return { content: [{ type: "text", text: `Described ${CATALOG_FIELD_CATALOG.length + ENGAGEMENT_FIELD_CATALOG.length} reportable fields across two builder sources and ${workspaceOutline(current).length} page blocks.` }], structuredContent: { schemaVersion: "adaptive-interfaces.workspace/v3", sources: [{ name: "steam_catalog", label: "Steam product catalog", recordCount, fields: CATALOG_FIELD_CATALOG }, { name: "customer_engagement", label: "Customer engagement", views: ["sessions", "funnel"], fields: ENGAGEMENT_FIELD_CATALOG, sharedFilters: engagementFiltersRef.current, guidance: ["Use sessions for users, sessions, duration, device, product, shop, and customer analysis.", "Use funnel for ordered Visitors, Sign-ups, Active, and Subscribed stages.", "Set inheritPageFilters to true for reports that should respond to the builder's filter panel.", "Supplier maps to publisher, brand to developer, productCategory to genre, and productClass to Steam category."] }], workspace: { storage: "local", pageTitle: current.pageTitle, audience: current.audience, onboarding: current.onboarding, selectedBlockId: current.selectedBlockId, blocks: workspaceOutline(current) }, htmlBindings: HTML_BINDINGS, compositionGuide: PAGE_COMPOSITION_GUIDE, spans: SPANS, composeOperations: ["inspect", "setPageTitle", "select", "addHtml", "addTabs", "move", "setSpan", "configure", "remove", "undo", "reset"], reportDefinition: { data: REPORT_DATA_SCHEMA, presentation: REPORT_PRESENTATION_SCHEMA, valueFormats: VALUE_FORMATS }, presentationModes: REPORT_MODE_CATALOG, guidance: ["Use onboard_audience for the name, company, and role survey. After submission, propose the page and wait for approval before request_page_composition.", "Start each approved composition with setPageTitle so the builder and browser tab describe the page purpose.", "Use create_report for every data-derived question, calculation, ranking, comparison, summary, table, chart, or narrative.", REPORT_PRESENTATION_DESCRIPTION] } }; } },
    ...
    const tool = {
      ...
      execute: (input: Record<string, unknown>) => {
        ...
        return {
          ...
          structuredContent: {
            schemaVersion: "adaptive-interfaces.page-creation-request/v2",
            requestedAction: "create_page",
            audience: current.audience,
            approvedProposal: proposal,
            compositionGuide: PAGE_COMPOSITION_GUIDE,
            instruction: "Create the approved page now with compose_page and create_report. First set a concise page title with setPageTitle, then follow the approved purpose, section order, and primary action.",
          },
        };
      },
    };
  ...
  const pageHeader = !onboardingActive ? <div className={`builder-top-page-header ${editMode ? "edit-mode" : "view-mode"}`} ref={widgetPromptMenuRef}>
    <div className="builder-top-page-copy">
      <h2 id="workspace-title">{pageTitle}</h2>
    </div>
